...
option1 = st.sidebar.button('(예정) siic 월간보고')
option2 = st.sidebar.button('(예정) siic 운영실적')
option3 = st.sidebar.button('siic 콜건수예측')
option4 = st.sidebar.button('(예정) siic 데이터분석')

selected_option = None
if option1:
    selected_option = '(예정) siic 월간보고'
elif option2:
    selected_option = '(예정) siic 운영실적'
elif option3:
    selected_option = 'siic 콜건수예측'
elif option4:
    selected_option = '(예정) siic 데이터분석'
...
# 선택된 옵션에 따라 다른 내용 표시
if selected_option == '(예정) siic 월간보고':
    st.header('SIIC 월간보고')
    st.write('Here is the monthly report content...')

elif selected_option == '(예정) siic 운영실적':
    st.header('SIIC 운영실적')
    st.write('Here is the operation performance content...')

elif selected_option == 'siic 콜건수예측':
    st.header('SIIC 콜건수예측')
    call_forecast()

elif selected_option == '(예정) siic 데이터분석':
    st.header('SIIC 데이터분석')
    st.write('Here is the data analysis content...')
